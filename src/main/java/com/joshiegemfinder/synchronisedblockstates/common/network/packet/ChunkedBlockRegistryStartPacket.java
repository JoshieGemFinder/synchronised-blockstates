...
public record ChunkedBlockRegistryStartPacket(UUID uuid, int classTableSize, int stringTableSize, int totalPropertyCount, int totalBlockCount) {
	...
	public static void encode(FriendlyByteBuf buf, ChunkedBlockRegistryStartPacket packet) {
		...
		buf.writeInt(packet.classTableSize());
		buf.writeInt(packet.stringTableSize());
	...
	public static ChunkedBlockRegistryStartPacket decode(FriendlyByteBuf buf) {
		...
		int classTableSize = buf.readInt();
		int stringTableSize = buf.readInt();
		int totalPropertyCount = buf.readInt();
		int totalBlockCount = buf.readInt();
		return new ChunkedBlockRegistryStartPacket(uuid, classTableSize, stringTableSize, totalPropertyCount, totalBlockCount);
	}
